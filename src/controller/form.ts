...
class FormController {
    ...
    public async Post(req: Request, res: Response) {
        try {
            ...
            const form = await AppDataSource.getRepository(FormEntity ).createQueryBuilder().insert().into(FormEntity ).values({ name,phone_number,description }).returning("*").execute()
    ...
    public async Put(req: Request, res: Response) {
        try {
            ...
            const form = await AppDataSource.getRepository(FormEntity ).createQueryBuilder().update(FormEntity )
                .set({ name,phone_number,description })
                .where({ id })
                .returning("*")
                .execute()
